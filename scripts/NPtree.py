...
SQL语句
sqlite3占位符语法
Table name: np_classifier_enhanced_test
"""
# 输入为id查询化合物分类
chem_tabel = "np_classifier_enhanced_test"
sql_find_classify_by_id = f"""select pathway, superclass, class from np_classifier_enhanced_test where id = :id """
sql_find_id_by_inchikey = """select id from np_classifier_enhanced_test where inchikey = :inchikey """
sql_find_id_by_inchi = """select id from np_classifier_enhanced_test where inchi = :inchi """
sql_find_id_by_smiles = """select id from np_classifier_enhanced_test where smiles = :smiles """
sql_all_info = """select * from np_classifier_enhanced_test where id = :id """
sql_table_info = """PRAGMA table_info(np_classifier_enhanced_test);"""
...
def connect_db():
    ...
    """
    # 数据库路径
    # db_file = 'data/ichem_np.db'
    db_file = os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), 'data/ichem_np.db')
    if not os.path.exists(db_file):
        user_input = input(f"Necessary database resources (587.3 MB) not found. Do you want to download it? (y/n): ")
        if user_input.lower() == 'y':
            url = 'https://iphylo.net/resource/ichem_np_db'
